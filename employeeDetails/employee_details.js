...
function findEmployeeBySpecialization(specialization) {
        const findEmployee = employees.find(employee => employee.specialization === specialization);
        if (findEmployee) {
        document.getElementById('employeesDetails').innerHTML =`<p>${findEmployee.id}: ${findEmployee.name}: ${findEmployee.name} - ${findEmployee.department} - $${findEmployee.salary}</p>`;
        }
        else{
